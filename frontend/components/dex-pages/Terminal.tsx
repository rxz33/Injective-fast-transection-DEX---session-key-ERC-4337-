...
export default function Terminal() {
    ...
    const chartData = useMemo(() => {
        ...
        const factor = timeframe === "1M" ? 1 : timeframe === "5M" ? 2 : timeframe === "15M" ? 3 : 5;
        ...
        <div className="mx-auto max-w-7xl space-y-6 pb-20">
            ...
            <div className="flex flex-wrap items-center justify-between gap-4 glass-card p-4">
                ...
                <div className="flex items-center gap-6 overflow-x-auto">
                    ...
                    <div className="flex flex-col">
                        <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Vault Balance</span>
                        <span className={`font-mono text-lg ${balanceLoading ? "text-accent animate-pulse" : "text-accent"}`}>{vaultInjBalance} <span className="text-[10px]">INJ</span></span>
                    </div>
                </div>
                        ...
                        <div className="flex gap-2">
                            {["1M", "5M", "15M", "1H", "1D"].map((t) => (
                                <button
                                    ...
                                    className={`px-2 py-1 text-[10px] font-bold rounded transiton-all ${t === timeframe ? "bg-accent/20 text-accent ring-1 ring-accent/30" : "text-muted-foreground hover:bg-white/5"}`}
                                >
                                    {t}
                                </button>
                            ))}
                    ...
                    <div className="flex-1 relative bg-black/20 rounded-2xl border border-white/5 p-4 overflow-hidden mt-4">
                        <ResponsiveContainer width="100%" height="100%">
                            <AreaChart data={chartData}>
                                ...
                                <Tooltip
                                    contentStyle={{ backgroundColor: '#0f172a', borderColor: 'rgba(255,255,255,0.1)', borderRadius: '12px', boxShadow: '0 10px 30px rgba(0,0,0,0.5)' }}
                    ...
                    <div className="glass-card p-6 flex flex-col gap-6">
                            ...
                            <div className="space-y-2">
                                <div className="flex justify-between text-[10px] font-bold text-muted-foreground uppercase tracking-tighter">
                                    ...
                                    <div className="flex items-center gap-1">
                                        <span>Available:</span>
                                        <span className={balanceLoading ? "text-accent animate-pulse" : "text-foreground"}>{vaultInjBalance} INJ</span>
                                    </div>
                                </div>
                        ...
                        <div className="space-y-3 pt-2">
                            <AnimatedGenerateButton
                                ...
                                labelIdle={Number(vaultInjBalance) === 0 ? "ZERO BALANCE" : (Number(buyQty) > Number(vaultInjBalance) ? "INSUFFICIENT BALANCE" : "FAST BUY")}
                                labelActive="EXECUTING"
                                generating={pending}
                                highlightHueDeg={215}
                                showIcon={false}
                                disabled={pending || Number(buyQty) > Number(vaultInjBalance) || Number(vaultInjBalance) === 0}
                                onClick={() => runTrade(0, buyQty)}
                            />
                            <AnimatedGenerateButton
                                className="w-[220px] h-20"
                                labelIdle={Number(vaultInjBalance) === 0 ? "ZERO BALANCE" : (Number(sellQty) > Number(vaultInjBalance) ? "INSUFFICIENT BALANCE" : "FAST SELL")}
                                labelActive="EXECUTING"
                                generating={pending}
                                highlightHueDeg={350}
                                showIcon={false}
                                disabled={pending || Number(sellQty) > Number(vaultInjBalance) || Number(vaultInjBalance) === 0}
                                ...
                                <div className="text-right">
                                    {row.txHash ? (
                                        <a
                                            ...
                                            className="text-[10px] font-bold tracking-widest text-accent uppercase hover:underline"
                                        >
                                            Success
                                        </a>
                                    ) : (
                                        <span className="text-[10px] font-bold tracking-widest text-accent/50 uppercase">Success</span>
                                    )}
                                </div>
                            </div>
                        ))}
